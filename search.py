...
def lattice(search_polygon, base_polygon, max_N):
    ...
    while N <= max_N:
        ...
        # set configurations
        configs = lattice.get_lattice_points(4, N)
...
def montecarlo(search_polygon, base_polygon, max_N):
    ...
    for i in range(0, i_max + 1):
        ...
        scale = [polygon.get_possible_scale(
            search_polygon, base_polygon, x[j], y[j], angle[j]) for j in range(0, N)]
